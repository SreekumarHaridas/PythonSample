...
# FIRST THIS PROGRAM CHOOSE SUPER STORMS THEN CHOOSE INTENSE STORMS, WHICH DOES NOT OVER LAP WITH ANY OF THE
# SUPER STORMS AND FINALLY PICK UP MODERATE STORMS, WHICH ARE NOT COINCIDE WITH THOSE SUPER STORMS AND INTENSE STORMS.
import time
start = time.time()
import shutil
import os
...
with open("INPUT_LOG_STORM_CLASSES_SEPARATOR_V2_EVENT_WISE.txt", 'w') as f1:
...
try:
    ...
    if DATA2.empty == True:
        ...
        STORM_RANGE_SET = {}
        for MEMBER in ST_DATES_LIST:
            MEMBER = datetime.strptime(MEMBER, '%Y-%m-%d')
            TWO_DAYS_BACK = MEMBER-dt.timedelta(days=5)
            TWO_DAYS_BACK = TWO_DAYS_BACK.strftime('%Y-%m-%d')
            THREE_DAYS_FRONT =MEMBER+dt.timedelta(days=6)
            THREE_DAYS_FRONT = THREE_DAYS_FRONT.strftime('%Y-%m-%d')
            STORM_RANGE = (pd.date_range(start=TWO_DAYS_BACK, end=THREE_DAYS_FRONT)).strftime('%Y-%m-%d')
            STORM_RANGE_SET[MEMBER] = list(STORM_RANGE)
        STORM_RANGE1 = STORM_RANGE_SET.values()
        STORM_RANGE2old = list(STORM_RANGE1)
        STORM_RANGE2 = sum(STORM_RANGE2old, [])
        STORM_RANGE3 = set(STORM_RANGE2)
        STORM_RANGE4 = sorted(STORM_RANGE3)
        STORM_RANGE4 = set(STORM_RANGE4)
    ...
    if DATA3.empty == True:
        ...
        ST_DATE48 = ST_DATE481-STORM_RANGE4
        ST_DATES_LIST8 = sorted(ST_DATE48)
        STORM_RANGE_SET8 = {}
        for MEMBER8 in ST_DATES_LIST8:
            MEMBER8 = datetime.strptime(MEMBER8, '%Y-%m-%d')
            TWO_DAYS_BACK8 = MEMBER8 - dt.timedelta(days=5)
            TWO_DAYS_BACK8 = TWO_DAYS_BACK8.strftime('%Y-%m-%d')
            THREE_DAYS_FRONT8 = MEMBER8 + dt.timedelta(days=6)
            THREE_DAYS_FRONT8 = THREE_DAYS_FRONT8.strftime('%Y-%m-%d')
            STORM_RANGE8 = (pd.date_range(start=TWO_DAYS_BACK8, end=THREE_DAYS_FRONT8)).strftime('%Y-%m-%d')
            STORM_RANGE_SET8[MEMBER8] = list(STORM_RANGE8)
        STORM_RANGE18 = STORM_RANGE_SET8.values()
        STORM_RANGE2old8 = list(STORM_RANGE18)
        STORM_RANGE28 = sum(STORM_RANGE2old8, [])
        STORM_RANGE38 = set(STORM_RANGE28)
        STORM_RANGE48 = sorted(STORM_RANGE38)
        STORM_RANGE48 = set(STORM_RANGE48)
    ...
    if DATA4.empty == True:
        ...
        ST_DATE491 = ST_DATE4911-STORM_RANGE4
        ST_DATE49 = ST_DATE491-STORM_RANGE48
