...
with open("INPUT_LOG_STORM_DAYS_COLUMN_WISE_DATA_SEPARATOR_V3_M2_IST_FOR_ACARR.txt", 'w') as f1:
    str_input_log = repr(input_log)
    f1.write("input_log = " + str_input_log + "\n")
DATA_PATH = PWD+'OMNI_HRO_1MIN_8310.csv'
...
try:
    ...
    axs = gs.subplots(sharex=True, sharey=False)
    axs[0].plot(DATA['BZ__GSM_nT'], lw=1.2, color='black')
    axs[0].set_ylabel("IMF B$_{Z}$ (nT)", fontsize=22, fontweight="bold")
    axs[0].yaxis.set_label_position("left")
    axs[0].tick_params(direction='in', labelsize=20, length=2, width=2, grid_alpha=0.5, right=False,
                       labelright=False, left=True, labelleft=True)
    axs[0].xaxis.grid(which='both', linestyle=':', linewidth=0.4)
    axs[0].margins(x=0)

    axs[1].plot(DATA['SYM/H_INDEX_nT'], lw=1.2, color='red')
    axs[1].set_ylabel("SYM-H (nT)", fontsize=20, fontweight="bold")
    axs[1].yaxis.set_label_position("left")
    axs[1].tick_params(direction='in', labelsize=20, length=2, width=2, grid_alpha=0.5, right=False,
                       labelright=False, left=True, labelleft=True)
    axs[1].xaxis.grid(which='both', linestyle=':', linewidth=0.4)
    axs[1].tick_params(axis='x', which='minor', labelsize=20)
    ...
    axs[1].margins(x=0)
    axs[1].set_xlabel('Date and Time (IST)', fontsize=20, fontweight="bold")
    fig.tight_layout()
    ...
    gs = fig1.add_gridspec(2, hspace=.1)
    axs = gs.subplots(sharex=True, sharey=False)
    axs[0].plot(DATA['BZ__GSM_nT'], lw=1.2, color='black')
    axs[0].set_ylabel("IMF B$_{Z}$ (nT)", fontsize=22, fontweight="bold")
    axs[0].yaxis.set_label_position("left")
    ...
    axs[1].plot(DATA['SYM/H_INDEX_nT'], lw=1.2, color='red')
    axs[1].set_ylabel("SYM-H (nT)", fontsize=22, fontweight="bold")
    axs[1].yaxis.set_label_position("left")
    axs[1].tick_params(direction='in', labelsize=20, length=2, width=2, grid_alpha=0.5, right=False,
                       labelright=False, left=True, labelleft=True)
    axs[1].xaxis.grid(which='both', linestyle=':', linewidth=0.4)
...
# -----------------------------------------ON 09-07-2021---------------------------SREEKUMAR HARIDAS--------------------